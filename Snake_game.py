...
class Snake_game(gym.Env):
    ...
    REWARD_PER_STEP_TOWARDS_FOOD = 2 #give reward for moving towards food and penalty for moving away
    REWARD_PER_FOOD = 50 
    REWARD_PER_STEP_REPEAT_LAST_FOUR = -1 #penalty for repeating the last four steps
    MAX_STEPS_AFTER_FOOD = 50 #stop if we go too long without food to avoid infinite loops
    ...
    def __init__(self, grid_size=12):
        ...
        # Save the snake trace
        self.snake_trace = self.snake_coordinates.copy()
        #Init the grid
    ...
    def step(self, action):
        ...
        self.snake_trace.append( (new_coord[0],new_coord[1]) ) #convert to tuple so we can use it to index
        if len(self.snake_trace) > 10:
            self.snake_trace = self.snake_trace[1:]
...
#                 reward += self.REWARD_PER_STEP
        ...
        #Penalty for repeating the last four steps
        if len(self.snake_trace) > 4:
            if (self.snake_trace[-1] == self.snake_trace[-5]):
                reward += self.REWARD_PER_STEP_REPEAT_LAST_FOUR        
